    ...
    for (const [month, transactions] of Object.entries(monthlyTransactions)) {
      ...
      const transactionStrings = transactions
        .map((t) => {
          ...
          // Format clearly for the LLM to read
          return `- Date: ${formattedDate}, Desc: ${t.description}, Amount: $${t.amount.toFixed(2)}, Balance: $${t.balance.toFixed(2)}`;
        })
        .join('\n');

      // Title is also month-first
      chunks.push(
        `Document Title: ${month} Transaction History for ${account.account_name}\nContent:\n${transactionStrings}`,
      );
    }
  } else {
